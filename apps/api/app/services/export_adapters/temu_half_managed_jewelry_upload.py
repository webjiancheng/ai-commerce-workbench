...
class TemuHalfManagedJewelryUploadAdapter:
    ...
    def write_excel(
        ...
        *,
        ...
        template_path = template_file_path or get_builtin_temu_upload_template_path()
        ...
        common_field_map = {
            item["field_name"]: item["column_index"]
            for item in meta["common_fields"]
        }
        detail_field_map = {
            item["field_name"]: item["column_index"]
            for item in meta["detail_fields"]
        }

        for field_name, value in common_fields.items():
            column_index = common_field_map.get(field_name)
            if not column_index:
                continue
            ws.cell(row=meta["common_values_row"], column=column_index).value = value

        start_row = meta["data_start_row"]
